...
class UpdateValue:
    ...
    def update_inner_value(self, mutable_dict: dict, keys_list: list, value: any) -> dict:
    ...
    def replacing_items_in_a_list(self, a_list: list, old_value: any, new_value: any) -> list:
        """Метод заменяет одно значение в списке на новое.Список не меняется."""
        a_list = [new_value if x == old_value else x for x in a_list]
        return a_list

    def replacement_by_specific_indices_list(self, a_list: list, b_list: list, value: any) -> list:
    ...
    def multiple_different_substitutions_list(self, a_list: list, b_list: list, c_list: list) -> list:
    ...
    def getting_data_by_indexes_from_a_list(self, a_list: list, indices_to_replace: list) -> list:
        """Возвращает список элементов из определенного списка по заданным индексам"""
        list_data = [a_list[x] for x in indices_to_replace]
        return list_data

    def getting_data_by_indexes_gen(self, a_list: list, indices_to_replace: list) -> object:
    ...
    def info_doc(self) -> None:
        ...
        p()
        text = ["Принимает на вход словарь, список и значение",
