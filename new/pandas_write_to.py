...
def to_csv_1() -> None:
    """Запись в csv"""
    pd.DataFrame({'col1': [1, 2], 'col2': [3, 4]}).to_csv('albums.csv',
                                                          index=False)
    p("""Файл записан""")
...
def to_csv() -> None:
    ...
    name = ['Rocka Rolla', 'Sad Wings of Destiny', 'Sin After Sin',
...
def to_xlsx() -> None:
    """Запись в xlsx"""
    # Создаем датафрейм
    data = {'Имя': ['Анна', 'Петр', 'Мария', 'Rocka Rolla',
                    'Sad Wings of Destiny', 'Sin After Sin', 'Stained Class',
                    'Killing Machine'],
            'Возраст': [25, 30, 35, 1974, 1976, 1977, 1978, 1978],
            'Город': ['Москва', 'Санкт-Петербург', 'Киев', 'a', 'b', 'c',
                      'd', 'e']}
